...
(function() {
    ...
    $(document).ready(function() {
            ...
            if (passengerChecked && passengerChecked.length > 0) {
                ...
                for (var h = l - 1; h >= 0; h--) {
                    ...
                    if (n.passenger_type != "3") {
                        ...
                        var g = m.length;
                        for (var e = 0; e < g; e++) {
                            if (m[e].nextSibling.nodeValue == f) {
                                m[e].checked = false
                            }
                        }
        ...
        $("#clearAll").click(function() {
            ...
            for (var e = 1; e < f; e++) {
                $("#date-list input[value=" + rqChecked[e] + "]").prop("checked", false)
            }
    ...
    jQuery.extend({
        init_select_date: function() {
            var f = $("#date-list").html();
            for (var g = 0; g < 5; g++) {
                var d = otherDateArr.length;
                var e = otherDateArr[d - 1];
                var k = new Date(Date.parse(e.replace(/-/g, "/")));
                ...
                c = j + "-" + h + "-" + l;
                otherDateArr.push(c);
                f += '<li><input type="checkbox" class="check" value="' + c + '"/><span>&nbsp;' + c + "</span></li>"
            }
            $("#date-list").html(f)
        },
        checkedPasseanger: function(h) {
            if (passengerAll) {
                var e = passengerAll.length;
                for (var d = 0; d < e; d++) {
                    var g = passengerAll[d];
        ...
        showSelectSeat: function() {
            ...
            var f = $("#seat-list input");
            for (var g = 0; g < n.length; g++) {
                f.eq(n[g]).attr("disabled", "disabled");
                $(f.eq(n[g])).parent().attr("class", "color999")
            }
            for (var g = 0; g < c.length; g++) {
                f.eq(c[g]).removeAttr("disabled");
                f.eq(c[g]).parent().removeClass("color999")
            }
            $("#sel-seat").show();
            $("#sel-buyer").hide();
            $("#sel-date").hide();
        ...
        removeSel: function(h, j, c) {
            $(h).parent().remove();
            if ("1" == c) {
                var g = $("#buyer-list input");
                var f = g.length;
                for (var d = 0; d < f; d++) {
                    if (g[d].nextSibling.nodeValue == j) {
                        g[d].checked = false;
                        $.removePasseanger(j)
                    }
                }
...
(function(a) {
    a.autoSubmit = function(c) {
        var d = this;
        var b = ["YZ", "ZE", "YW", "ZY", "RZ", "RW", "TZ", "GR", "SWZ", "WZ"];
        d.init = function() {
        ...
        d.getXBSize = function(h, g, l) {
            var j = 0;
            var i = g.length;
            for (var e = 0; e < i; e++) {
                var f = h.queryLeftNewDTO[g[e] + "_num"];
                if (f != "--" && f != "无") {
                    if (f == "有") {
                        if (l) {
                            l.cTypes.push(g[e])
